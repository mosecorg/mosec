...
- Try to split the whole inference process into different CPU and GPU stages (ref [DistilBERT](https://mosecorg.github.io/mosec/examples/pytorch.html#natural-language-processing)). The pipeline will make different stages run in parallel, which will fully utilize the GPU resource. Usually, higher GPU utilization will result in higher throughput. You can also adjust the number of workers in each stage to ensure that the GPU workers can get enough requests. Note that data passing through different stages will be serialized/deserialized by the `serialize_ipc/deserialize_ipc` methods, extremely large data might make the whole pipeline slower than before.
- Use an appropriate `serialize/deserialize` method. By default, it's JSON. However, images and embeddings are not well supported by JSON. You can choose msgpack which is faster and binary compatible (ref [Stable Diffusion](https://mosecorg.github.io/mosec/examples/stable_diffusion.html)).
